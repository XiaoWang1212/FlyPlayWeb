...
"""
    ...
    def generate_itinerary(
        ...
        try:
            ...
            pace_rule = (
                "行程緊湊度：緊湊，每日可用觀光時間為 8～10 小時，請安排景點使每日建議停留時間總和在 8 至 10 小時之間（含景點間移動時間），景點數不得超過 5 個。"
                if pace_label == "緊湊"
                else "行程緊湊度：輕鬆，每日可用觀光時間為 5～7 小時，請安排景點使每日建議停留時間總和在 5 至 7 小時之間（含景點間移動時間），景點數不得超過 4 個。"
            )
            ...
            - 輕鬆：每日可用觀光時間 5～7 小時，景點建議停留時間總和在 5 至 7 小時之間，景點數不超過 3 個
            - 緊湊：每日可用觀光時間 8～10 小時，景點建議停留時間總和在 8 至 10 小時之間，景點數不超過 5 個
            若單一景點建議停留時間已達 5 小時以上，該天不得再安排其他景點。
            【輕鬆行程最少行程數規則】行程緊湊度為「輕鬆」時，每天至少要安排 2 個景點。唯一例外是當天有主題樂園、大型遊樂園等建議停留時間達 5 小時以上的大型設施，才允許當天只有 1 個行程。廟宇、神社、寺院、公園、城跡、歷史遺址等一般景點的停留時間通常為 1～2 小時，絕對不得作為輕鬆行程當天的唯一行程；若安排了這類景點，必須再補充至少一個景點。
            {pace_rule}
            【絕對禁止】行程中不得包含任何餐廳、美食、小吃、夜市、食堂、咖啡廳、飲料店、市場（以飲食為主的市場）等飲食類景點。所有景點必須是觀光景點、文化設施、自然景觀、購物商圈（非美食市場）、遊樂設施等非餐飲類地點。
            days[].location.location_name裡面只可以有景點名稱，不要包含任何描述或其他資訊。
            所有地點名稱必須是 Google Maps 上可以直接搜尋到的具體地點，不得使用泛稱。
