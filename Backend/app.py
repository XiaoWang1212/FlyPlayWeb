...
def create_app():
    ...
    def get_key():
        return jsonify({"key": app.config.get("GOOGLE_MAPS_API_KEY")})
    ...
    def not_found(error):
        """404 錯誤處理"""
        return jsonify({"success": False, "error": "API端點未找到"}), 404
    ...
    def handle_exception(error):
        """全局異常處理"""
        return jsonify({"success": False, "error": f"未預期的錯誤: {str(error)}"}), 500
    ...
    def get_latlng():
        try:
            ...
            if not itinerary_id:
                return (
                    jsonify({"success": False, "error": "缺少 itinerary_id 参数"}),
                    400,
                )
            ...
            if result["success"]:
                try:
                    travel_service.update_itinerary_ai_data(
                        int(itinerary_id), data_latlng=result["data"]
                    )
    ...
    def clear_cache():
        ...
        return jsonify({"success": True, "message": "所有快取已清除"}), 200
    ...
    def generate_itinerary_detail():
        ...
        try:
            ...
            print(traceback.format_exc())
            return (
                jsonify({"success": False, "error": f"詳細行程生成失敗: {str(e)}"}),
                500,
            )
    ...
    def parse_itinerary():
        try:
            ...
            if not request_data:
                return (
                    jsonify({"success": False, "error": "請求內容為空或不是合法 JSON"}),
                    400,
                )
            ...
            if "days" in request_data and isinstance(request_data["days"], list):
                ...
                return (
                    jsonify(
                        {"success": False, "error": "請求格式錯誤，必須包含 days 陣列"}
                    ),
                    400,
                )
            ...
            if itinerary_id:
                try:
                    existing_itinerary = travel_service.get_itinerary(int(itinerary_id))
                    detailed = (
                        existing_itinerary.get("detailed_itinerary")
                    ...
                    if isinstance(detailed.get("parsed"), dict):
                        detailed["parsed"]["days"] = modified_days
                    else:
                        detailed["days"] = modified_days

                    travel_service.update_itinerary_ai_data(
                        int(itinerary_id), detailed_itinerary=detailed
                    )
                except Exception as save_err:
                    print(f"儲存補圖 detailed_itinerary 失敗: {save_err}")
            ...
            print(f"行程解析錯誤: {e}")
            print(traceback.format_exc())
            return jsonify({"success": False, "error": f"行程解析失敗: {str(e)}"}), 500
